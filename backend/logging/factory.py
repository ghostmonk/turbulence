...
from .interfaces import Logger, LoggerFactory, LogProvider, LogLevel, LogContext, LogEntry
...
class DefaultLogger(Logger):
    ...
    def log_request(
        ...
        **context
        ...
        level = LogLevel.ERROR if status and status >= 500 else \
                LogLevel.WARNING if status and status >= 400 else \
                LogLevel.INFO
...
def get_logger(component: str, **default_context) -> Logger:
    ...
    if _logger_factory is None:
        raise RuntimeError(
            "Logger factory not initialized. Call create_logger_factory() first."
        )
