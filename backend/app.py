...
class LoggingMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        logger.info_with_context(f"Request started: {request.method} {request.url.path}", {
        ...
        try:
            ...
            logger.info_with_context(f"Request completed: {request.method} {request.url.path}", {
                "request_id": request_id,
            ...
            process_time = time.time() - start_time
            logger.exception_with_context(f"Request failed: {request.method} {request.url.path}", {
                "request_id": request_id,
                "processing_time_ms": round(process_time * 1000, 2),
...
async def http_exception_handler(request: Request, exc: StarletteHTTPException):
    ...
    logger.error_with_context(f"HTTP exception: {exc.status_code}", {
        "path": request.url.path,
        "method": request.method,
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    logger.error_with_context("Request validation error", {
        "path": request.url.path,
        "method": request.method,
        "client_host": request.client.host if request.client else "unknown",
...
async def general_exception_handler(request: Request, exc: Exception):
    ...
    logger.exception_with_context("Unhandled exception", {
        "path": request.url.path,
        "method": request.method,
        "error_type": type(exc).__name__,
        "error_message": str(exc),
